...
| `a11y-baseline.spec.js` | baseline axe (desktop, tema claro), inclui drawer (tablet), rail expandido (notebook), a tela Dados e backup (1-F1), os estados de Ferramentas (1-F2) e os de Colaboradores (1-F3: filtros, menu em cada linha, formulário vazio e com erro, histórico, vazio, erro e escuro), com varredura restrita à tela **sem exclusão** |
| `visual-baseline.spec.js` | 4 capturas desktop + 4 da tela Dados e backup + 3 de Ferramentas + 3 de Colaboradores (menu aberto, perfil padrão, tema escuro) |
